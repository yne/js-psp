...
static void read_esds_atom(mp4info_t* info, const uint64_t total_size) {
	...
	if ( info->tracks[info->total_tracks-1]->audio_type == ATOM_TYPE('m','p','4','a') ) {
		...
		info->tracks[info->total_tracks-1]->mp4v_decinfo = (uint8_t*)js_malloc(info->tracks[info->total_tracks - 1]->mp4v_decinfo_size);
...
static void read_avcC_atom(mp4info_t* info, const uint64_t total_size) {
	...
	info->tracks[info->total_tracks - 1]->avc_sps = (uint8_t*)js_malloc(info->tracks[info->total_tracks - 1]->avc_sps_size);
	if ( info->tracks[info->total_tracks - 1]->avc_sps ) {
		...
		info->tracks[info->total_tracks - 1]->avc_pps = (uint8_t*)js_malloc(info->tracks[info->total_tracks - 1]->avc_pps_size);
...
static void read_stts_atom(mp4info_t* info, const uint64_t total_size) {
	...
	current_track->stts_sample_count = (uint32_t*)js_malloc(current_track->stts_entry_count * sizeof(uint32_t));
	current_track->stts_sample_duration = (uint32_t*)js_malloc(current_track->stts_entry_count * sizeof(uint32_t));

	if ( !(current_track->stts_sample_count) || !(current_track->stts_sample_duration) ){
		if (current_track->stts_sample_count) {
			js_free(current_track->stts_sample_count);
			current_track->stts_sample_count = 0;
		}
		if (current_track->stts_sample_duration) {
			js_free(current_track->stts_sample_duration);
...
static void read_ctts_atom(mp4info_t* info, const uint64_t total_size) {
	...
	current_track->ctts_sample_count = (uint32_t*)js_malloc(current_track->ctts_entry_count * sizeof(uint32_t));
	current_track->ctts_sample_offset = (uint32_t*)js_malloc(current_track->ctts_entry_count * sizeof(uint32_t));

	if ( !(current_track->ctts_sample_count) || !(current_track->ctts_sample_offset) ){
		if (current_track->ctts_sample_count) {
			js_free(current_track->ctts_sample_count);
			current_track->ctts_sample_count = 0;
		}
		if (current_track->ctts_sample_offset) {
			js_free(current_track->ctts_sample_offset);
...
static void read_stss_atom(mp4info_t* info, const uint64_t total_size) {
	...
	current_track->stss_sync_sample = (uint32_t*)js_malloc(current_track->stss_entry_count * sizeof(uint32_t));
...
static void read_stsc_atom(mp4info_t* info, const uint64_t total_size) {
	...
	current_track->stsc_first_chunk = (uint32_t*)js_malloc((current_track->stsc_entry_count+1) * sizeof(uint32_t));
	current_track->stsc_samples_per_chunk = (uint32_t*)js_malloc((current_track->stsc_entry_count+1) * sizeof(uint32_t));
	current_track->stsc_sample_desc_id = (uint32_t*)js_malloc((current_track->stsc_entry_count+1) * sizeof(uint32_t));
...
static void read_stsz_atom(mp4info_t* info, const uint64_t total_size) {
	...
	if ( current_track->stsz_sample_size == 0 ) {
		...
		current_track->stsz_sample_size_table = (uint32_t*)js_malloc(current_track->stsz_entry_count * sizeof(uint32_t));
...
static void read_stco_atom(mp4info_t* info, const uint64_t total_size) {
	...
	current_track->stco_chunk_offset = (uint32_t*)js_malloc(current_track->stco_entry_count * sizeof(uint32_t));
...
static void add_track(mp4info_t* info) {
	info->total_tracks++;
	info->tracks[info->total_tracks-1] = (mp4info_track_t*)js_malloc(sizeof(mp4info_track_t));
