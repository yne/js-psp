...
mp4info_t* mp4info_open(const char* filename) {
	mp4info_t* info = (mp4info_t*)js_malloc(sizeof(mp4info_t));
	if (!info)
		return 0;
	...
	for (i = 0; i < info->total_tracks; i++) {
		if (info->tracks[i]) {
			...
			if (info->tracks[i]->stts_sample_count)
				js_free(info->tracks[i]->stts_sample_count);
			if (info->tracks[i]->stts_sample_duration)
				js_free(info->tracks[i]->stts_sample_duration);
			if (info->tracks[i]->ctts_sample_count)
				js_free(info->tracks[i]->ctts_sample_count);
			if (info->tracks[i]->ctts_sample_offset)
				js_free(info->tracks[i]->ctts_sample_offset);
			if (info->tracks[i]->stss_sync_sample)
				js_free(info->tracks[i]->stss_sync_sample);
			if (info->tracks[i]->stsc_first_chunk)
				js_free(info->tracks[i]->stsc_first_chunk);
			if (info->tracks[i]->stsc_samples_per_chunk)
				js_free(info->tracks[i]->stsc_samples_per_chunk);
			if (info->tracks[i]->stsc_sample_desc_id)
				js_free(info->tracks[i]->stsc_sample_desc_id);
			if (info->tracks[i]->stsz_sample_size_table)
				js_free(info->tracks[i]->stsz_sample_size_table);
			if (info->tracks[i]->stco_chunk_offset)
				js_free(info->tracks[i]->stco_chunk_offset);
			
			if (info->tracks[i]->avc_sps)
				js_free(info->tracks[i]->avc_sps);
			if (info->tracks[i]->avc_pps)
				js_free(info->tracks[i]->avc_pps);
			if (info->tracks[i]->mp4v_decinfo)
				js_free(info->tracks[i]->mp4v_decinfo);
