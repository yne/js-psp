...
int OggCallbackSeek(void *ch,ogg_int64_t offset,int position){
	...
	if(!seekPos && !streamsOgg[*channel].autoloop){
		streamsOgg[*channel].paused=1;
	...
	while (streamsOgg[channel].bufLength<byteLength){
		...
		if (!bytesRead){
			if (!streamsOgg[channel].autoloop){
				streamsOgg[channel].paused=1;
				streamsOgg[channel].outputInProgress=0;
				streamsOgg[channel].stopReason=PSPAALIB_STOP_END_OF_STREAM;
...
int LoadOgg(char* filename,int channel,int loadToRam){
	...
	if (streamsOgg[channel].initialized)
		UnloadOgg(channel);
	loadToRam=0;//////Something wrong with the seek/read function.Can't rewind when on ram.
	...
	streamsOgg[channel].paused=1;
	streamsOgg[channel].initialized=1;
...
int UnloadOgg(int channel){
	...
	streamsOgg[channel].initialized=0;
	return PSPAALIB_SUCCESS;
}
