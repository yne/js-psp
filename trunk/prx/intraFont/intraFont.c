...
unsigned long* intraFontGetTable(FILE *file, unsigned long n_elements, unsigned long bp_element) {
	...
	unsigned long *table = (unsigned long*)js_malloc(n_elements*sizeof(unsigned long));
	if (table == NULL) {
		js_free(raw_table);
		return NULL;
	}
...
intraFont* intraFontLoad(const char *filename, unsigned int options) {
	...
	if (font->fileType == FILETYPE_PGF) {
		...
		font->glyph = (Glyph*)js_malloc(font->n_chars*sizeof(Glyph));
		font->glyphBW = NULL;
		font->shadowGlyph = (Glyph*)js_malloc(font->n_shadows*sizeof(Glyph));
		...
		font->glyphBW = (GlyphBW*)js_malloc(font->n_chars*sizeof(GlyphBW));
  ...
  font->options = options;
	...
	if (font->fileType == FILETYPE_PGF) {
		...
		//cache chars, swizzle texture and js_free unneeded stuff (if INTRAFONT_CACHE_ALL or _ASCII)
		sceKernelDcacheWritebackAll();
		...
		//js_free temp stuff: not needed -> skip
		//cache chars, swizzle texture and js_free unneeded stuff (if INTRAFONT_CACHE_ALL or _ASCII): not available ->skip
...
void intraFontUnload(intraFont *font) {
	if (!font) return;
    if (font->filename) js_free(font->filename);
    if (font->fontdata) js_free(font->fontdata);
	if (font->texture) js_free(font->texture);
	if (font->fileType == FILETYPE_PGF) {
		if (font->charmap_compr) js_free(font->charmap_compr);
		if (font->charmap) js_free(font->charmap);
		if (font->glyph) js_free(font->glyph);
		if (font->shadowGlyph) js_free(font->shadowGlyph);
	} else { //FILETYPE_BWFON
		if (font->glyphBW) js_free(font->glyphBW);
	}
	if (font) js_free(font);
	font = NULL;
}
...
#define LOCAL_BUFFER_LENGTH 256
	...
	if (length > LOCAL_BUFFER_LENGTH) ucs2_text = (cccUCS2*)js_malloc(length*sizeof(cccUCS2));
	...
	if (ucs2_text != buffer) js_free(ucs2_text);
    return 0.0f;
}
...
float intraFontPrintColumnUCS2Ex(intraFont *font, float x, float y, float column, const cccUCS2 *text, int length) {
	...
	if (font->options & INTRAFONT_SCROLL_LEFT) {
		for (i = 0; i < length; i++) {
			if (text[i] == '\n') {
				cccUCS2* ucs2_text = (cccUCS2*)js_malloc(length*sizeof(cccUCS2));
				if (!ucs2_text) return x;
	...
	//js_test((u32)v);
	//js_evaluateScript("sceGuStart(0);sceGuClearColor(0x00444444);sceGuClear(5);sceGuFinish();sceGuSwapBuffers();");
			...
			if (column > 0.0f) {                                                
				if (font->options & INTRAFONT_SCROLL_LEFT) { //scrolling (any direction) requested
					...
					if (textwidth > column) {  //scrolling really required
						...
						js_evaluateScript("sceGuEnable(2)");//GU_SCISSOR_TEST
					}
	...
	if (!(font->options & INTRAFONT_ACTIVE)){
		...
		js_setProperty(IF,"v",I2J((void*)v));
		...
		js_evaluateScript("sceGuTexWrap(1,1)");//GU_CLAMP
	...
	js_evaluateScript("sceGuEnable(1)");//GU_DEPTH_TEST
	
	if (scroll == 1) {
		js_evaluateScript("sceGuScissor(0,0,480,272)"); //reset window to whole screen (test was previously enabled)
